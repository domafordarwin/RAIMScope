---
name: hwpx
description: "한글(HWP/HWPX) 문서 생성/읽기/편집 스킬. .hwp, .hwpx 파일, 한글 문서, Hancom, OWPML 관련 요청 시 사용."
---

# 한글 문서 스킬 — HWP + HWPX 지원

> **RAIMScope 설치 시 확인된 제약사항 (2026-07-26, `SkillUp/과학실개선/hwpx-skill`에서 이식, 스크립트별 실행 검증 완료):**
> - `scripts/create_document.py`, `scripts/text_extract.py`는 원본이 참조하는 `from hwpx import HwpxDocument, TextExtractor`가 실존하지 않는 모듈이라 **이식하지 않음** (PyPI에 이름만 같은 무관한 `hwpx` 1.1.1 패키지가 있으나 `TextExtractor`를 제공하지 않아 설치해도 소용없음 — 확인 후 제거함). 텍스트 추출이 필요하면 `scripts/office/unpack.py`로 풀어서 `Contents/section0.xml`의 `<hp:t>` 텍스트를 직접 읽을 것.
> - 원본 SKILL.md가 문서화했던 `scripts/hwp_text_extract.py`, `scripts/hwp_to_hwpx.py`(HWP 바이너리 지원), `examples/` 디렉토리, `templates/gonmun`·`templates/minutes`·`templates/proposal` 오버레이는 원본 디렉토리에 실물이 없어 **이식되지 않음** — 아래 워크플로우 3-B/3-C 및 gonmun/minutes/proposal 관련 서술은 참고용 스펙일 뿐 이 설치본에서는 동작하지 않는다. 실제 사용 가능한 템플릿은 `base`, `report` 뿐.
> - 원본의 `fill_plan.py`, `make_plan.sh`는 다른 프로젝트(송내중앙중학교 AI융합형교육실 계획서)에 특화된 1회성 스크립트라 이식 대상에서 제외함.
> - 실제 동작 검증됨: `build_hwpx.py`(report 템플릿 빌드), `validate.py`, `scripts/office/unpack.py`, `analyze_template.py`. 의존성 `lxml`, `olefile`은 `onion_yolo_project/.venv`에 설치 완료.

한글(Hancom Office)의 **HWPX** 파일을 XML 직접 작성 중심으로 생성, 편집, 읽기하는 스킬.
(위 제약사항에 따라 이 설치본은 **HWPX 전용**이며 HWP 바이너리 지원은 없음)

- **HWPX**: ZIP 기반 XML 컨테이너(OWPML 표준). 생성/편집/읽기 모두 지원.

## 환경

```
# SKILL_DIR는 이 SKILL.md가 위치한 디렉토리의 절대 경로
SKILL_DIR="/Users/science/workspace/RAIMScope/.claude/skills/hwpx"

# Python 가상환경 — RAIMScope는 onion_yolo_project/.venv를 공용으로 사용 (README.md 참고)
VENV="/Users/science/workspace/RAIMScope/onion_yolo_project/.venv/bin/activate"
```

모든 Python 실행 시:
```bash
# 프로젝트의 .venv를 활성화 (pip install lxml olefile 필요)
source "$VENV"
```
...
│   ├── hwp_text_extract.py               # HWP(바이너리) 텍스트 추출
│   └── hwp_to_hwpx.py                    # HWP → HWPX 변환
├── templates/
...
## 워크플로우 3-B: HWP(바이너리) 읽기/텍스트 추출

HWP 5.x 바이너리 파일에서 텍스트를 추출한다. `olefile` 패키지가 필요하다.

```bash
source "$VENV"

# 기본 텍스트 추출 (BodyText 레코드 파싱)
python3 "$SKILL_DIR/scripts/hwp_text_extract.py" document.hwp

# 마크다운 형식
python3 "$SKILL_DIR/scripts/hwp_text_extract.py" document.hwp --format markdown

# 빠른 추출 (PrvText 미리보기 스트림만 사용, 암호화 파일도 가능)
python3 "$SKILL_DIR/scripts/hwp_text_extract.py" document.hwp --preview-only

# 파일 정보 확인 (버전, 압축여부, 암호화여부, 스트림 목록)
python3 "$SKILL_DIR/scripts/hwp_text_extract.py" document.hwp --info

# 결과를 파일로 저장
python3 "$SKILL_DIR/scripts/hwp_text_extract.py" document.hwp -o output.txt
```
...
| 방법 | 옵션 | 장점 | 단점 |
|------|------|------|------|
| BodyText 파싱 | (기본) | 정확한 텍스트 추출 | 암호화 파일 불가 |
| PrvText 미리보기 | `--preview-only` | 빠르고 암호화 파일도 가능 | 전체 내용이 아닐 수 있음 |

### Python API

```python
from hwp_text_extract import extract_text, extract_preview_text, get_hwp_info

# 전체 텍스트 추출
text = extract_text("document.hwp")

# 빠른 미리보기 텍스트
preview = extract_preview_text("document.hwp")

# 파일 정보
info = get_hwp_info("document.hwp")
print(info["version"], info["compressed"], info["encrypted"])
```

---

## 워크플로우 3-C: HWP → HWPX 변환

HWP 바이너리 파일의 텍스트를 추출하여 HWPX 문서로 변환한다.

**주의**: 텍스트 수준의 변환이다. 표, 이미지, 복잡한 서식은 보존되지 않는다.
완전한 레이아웃 보존이 필요하면 한글 오피스의 "다른 이름으로 저장 → HWPX"를 사용할 것.

```bash
source "$VENV"

# 기본 변환
python3 "$SKILL_DIR/scripts/hwp_to_hwpx.py" document.hwp --output result.hwpx

# 템플릿 적용하여 변환
python3 "$SKILL_DIR/scripts/hwp_to_hwpx.py" document.hwp --template report --output result.hwpx

# PrvText 기반 빠른 변환 (암호화 파일도 가능)
python3 "$SKILL_DIR/scripts/hwp_to_hwpx.py" document.hwp --preview-only --output result.hwpx

# 메타데이터 설정
python3 "$SKILL_DIR/scripts/hwp_to_hwpx.py" document.hwp \
  --title "변환 문서" --creator "작성자" --output result.hwpx
```

### 변환 후 편집 패턴

HWP → HWPX 변환 후 기존 HWPX 워크플로우로 편집 가능:

```bash
# 1. HWP → HWPX 변환
python3 "$SKILL_DIR/scripts/hwp_to_hwpx.py" original.hwp --output converted.hwpx

# 2. HWPX를 unpack하여 XML 편집
python3 "$SKILL_DIR/scripts/office/unpack.py" converted.hwpx ./unpacked/
# (section0.xml 편집)
python3 "$SKILL_DIR/scripts/office/pack.py" ./unpacked/ edited.hwpx
...
| `scripts/hwp_text_extract.py` | **HWP** 바이너리 텍스트 추출 (olefile 기반) |
| `scripts/hwp_to_hwpx.py` | **HWP → HWPX** 텍스트 수준 변환 |
...
1. **HWP+HWPX 지원**: `.hwpx`는 생성/편집/읽기 모두 가능. `.hwp`(바이너리)는 읽기/텍스트 추출/HWPX 변환 지원. `.hwp` 편집이 필요하면 `hwp_to_hwpx.py`로 변환 후 HWPX 워크플로우 사용. 복잡한 서식 보존이 필요하면 한글 오피스의 "다른 이름으로 저장 → HWPX" 안내.
...
7. **venv 사용**: 프로젝트의 `.venv/bin/python3` (lxml, olefile 패키지 필요)
